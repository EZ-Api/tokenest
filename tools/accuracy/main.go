...
func main() {
	reportDirFlag := flag.String("report-dir", "", "write a markdown report to this directory (default: <repo>/tokenest/report)")
	flag.Parse()
	...
	if reportDir != "" {
		if err := writeReport(reportDir, header, rows); err != nil {
...
func writeReport(dir string, header []string, rows [][]string) error {
	if dir == "" {
		return nil
	}
